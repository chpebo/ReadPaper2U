...
- **다국어** — 인터페이스와 대화는 간체 중국어, 번체 중국어, 영어,
  한국어를 지원한다.
- **다크/라이트 테마** — 설정에서 전환할 수 있으며, 두 테마는 동일한
  CSS 변수 체계를 공유한다.
...
첫 실행 시 홈 화면의 **설정** 버튼을 누르고 다음 세 가지를 입력
한다:
...
넣으면 된다. 스크립트가 진행되는 동안에는 우측 상단의 톱니바퀴
아이콘으로 같은 설정 패널을 다시 열 수 있다.

### 오프라인 / 폐쇄망 환경

기본 설정에서 PDF.js와 KaTeX는 공개 CDN
(`cdnjs.cloudflare.com`, `cdn.jsdelivr.net`)에서 로드된다. CDN에
도달할 수 없을 때는 로컬 `lib/` 폴더로 자동 폴백된다. 완전한
오프라인 사용을 원하면 다음 파일들을 `index.html`과 같은 디렉터리에
배치한다:
...
해당 파일들은 PDF.js와 KaTeX의 공식 릴리스에서 받을 수 있으며,
[THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)에 자세한 안내가
있다.
...
서드파티 라이브러리 (PDF.js, KaTeX)는 각자의 라이선스를 따르며,
[THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)에 정리되어 있다.

---

## 감사의 말

[PDF.js](https://mozilla.github.io/pdf.js/) (Apache 2.0)와
[KaTeX](https://katex.org/) (MIT) 위에서 만들어졌다. LLM 추론은
사용자가 지정한 OpenAI 호환 엔드포인트가 담당한다. 기본 동반자
캐릭터인 Banni는 실제로 존재하는 고양이이다.
